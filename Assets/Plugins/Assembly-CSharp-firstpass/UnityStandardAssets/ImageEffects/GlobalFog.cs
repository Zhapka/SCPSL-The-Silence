...
namespace UnityStandardAssets.ImageEffects
{
    ...
    public class GlobalFog : PostEffectsBase
    {
        ...
        [Header("High Altitude Settings")]
        [Tooltip("Y coordinate threshold to push fog away")]
        public float altitudeThreshold = 800f;
        [Tooltip("How far the fog ends when high in the sky")]
        public float highAltitudeEndDistance = 5000f;
        [Tooltip("How fast the fog transitions to the far distance")]
        ...
        private bool hasSavedOriginals = false;
        ...
        private void OnRenderImage(RenderTexture source, RenderTexture destination)
        {
            ...
            // Save original project fog settings once at runtime/editor execution
            ...
            // Dynamically change fog distance based on camera height
            if (transform.position.y >= altitudeThreshold)
            {
                RenderSettings.fogEndDistance = Mathf.MoveTowards(RenderSettings.fogEndDistance, highAltitudeEndDistance, transitionSpeed * Time.deltaTime * 100f);
            }
            else
            {
                RenderSettings.fogEndDistance = Mathf.MoveTowards(RenderSettings.fogEndDistance, originalEndDistance, transitionSpeed * Time.deltaTime * 100f);
            }
            ...
            CustomGraphicsBlit(source, destination, fogMaterial, (!distanceFog || !heightFog) ? (distanceFog ? 1 : 2) : 0);
        }
